...
export const Header: React.FC = () => {
  ...
  return (
    <>
      ...
      >
        {/* Top Info Bar */}
        <div className={`hidden xl:flex justify-between items-center px-6 xl:px-10 py-1.5 transition-all duration-300 ${scrolled ? 'h-0 overflow-hidden opacity-0 py-0 border-none' : 'border-b border-white/10 opacity-100'}`}>
        ...
        <div className={`max-w-8xl mx-auto px-6 xl:px-10 flex items-center justify-between transition-all duration-300 ${scrolled ? 'py-0.5' : 'py-2'}`}>

          {/* Logo */}
          <Logo variant="light" className="w-28 xl:w-40 flex-shrink-0" />

          {/* Desktop Nav */}
          <nav className="hidden xl:flex items-center gap-1 bg-white/5 border border-white/10 rounded-full px-2 py-1.5 shadow-inner" aria-label="Main navigation">
            {navLinks.map((link) => {
